...
const LIB_CHANGELOG_PATH = path.join(LIB_DIR, "CHANGELOG.md");
if (fs.existsSync(LIB_CHANGELOG_PATH)) {
  const content = fs.readFileSync(LIB_CHANGELOG_PATH, "utf-8");
  const formatted = formatChangelog(content);
  if (formatted !== content) {
    fs.writeFileSync(LIB_CHANGELOG_PATH, formatted);
  }
}
