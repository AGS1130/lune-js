import { isArray, isObject } from "@lune-js/utils";
import { TrackOpTypes } from "./constants";
import { ARRAY_ITERATE_KEY, track } from "./dep";
import { reactive, readonly } from "./reactive";
import { endBatch, setActiveSub, startBatch } from "./system";
import type { DeepReadonly } from "./types";
import { isProxy, isReactive, isReadonly, isShallow, toRaw } from "./utils";
...
function toReactive<T extends unknown>(value: T): T {
  if (isObject(value)) {
    // @ts-expect-error: Type 'Reactive<T & object>' is not assignable to type 'T'.
 ...
 */
function toReadonly<T extends unknown>(value: T): DeepReadonly<T> {
  if (isObject(value)) {
    // @ts-expect-error: Type 'DeepReadonly<UnwrapNestedRefs<T & object>>' is not assignable to type 'DeepReadonly<T>'.
...
function iterator(self: unknown[], method: keyof Array<unknown>, wrapValue: (value: any) => unknown) {
  ...
  if (arr !== self && !isShallow(self)) {
    iter._next = iter.next.bind(iter);
    iter.next = () => {
...
function apply(
  ...
  // https://github.com/vuejs/core/issues/11759
  ...
  if (methodFn !== arrayProto[method as any]) {
    // @ts-expect-error: 'methodFn' is of type 'unknown'
